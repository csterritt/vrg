...
- Build the final `vrg` binary and smoke-run it end to end through the four representative outcomes, using the Issue #4 fake-rg harness for determinism: a successful browse (exit 0 after `q`), a no-results search (exit 1 after `q`), a fatal fake-rg outcome with no usable results (exit 2 after overlay dismissal with either `q` or `Esc`), and cancellation while searching (exit 130 with the child terminated and reaped and the terminal restored). Each smoke run asserts the exit status, terminal restoration, and stderr replay where applicable.
...
- **Manual**: from a clean checkout, run the three repository-wide gates and record the commands, outputs, and exit statuses; then build the final binary and run the four smoke scenarios with the fake-rg harness, recording each exit status and confirming the terminal is usable afterwards.
...
- [ ] Given the final `vrg` binary, then smoke runs of a successful browse (0), a no-results search (1), a fatal fake-rg outcome (2), and cancellation (130) succeed with terminal restoration and, where applicable, stderr replay.
