...
- **Automated**:
  ...
  - **Gated layout preparation**: with the layout worker held by a test gate after a resize, assert `ctrl+c` exits 130, `q` exits with the fixed status, `n` advances the cursor, and a second resize is accepted, all without releasing the gate.
