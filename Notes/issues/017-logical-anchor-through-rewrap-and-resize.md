...
- **Navigation to a cached file with a stale layout**: when the current file changes (Issue 13 cross-file navigation) and the newly current file is cached but has no installed layout matching the current (path, revision, text width, wrap mode), the model requests a prepared layout for it. This covers the case where a resize or wrap toggle requested a layout only for the then-current file, leaving other cached files' layouts stale. The navigation intent (entry reveal or saved-viewport) is carried by the model and commits when the matching layout installs, per Issue 28's two-stage contract. If the cached file already has a matching installed layout, no request is needed and the intent commits immediately.
...
- **Automated**:
  ...
  - **Gated layout preparation**: with the layout worker held by a test gate after a resize, assert `ctrl+c` exits 130, `q` exits with the fixed status, `n` advances the cursor, `p` moves the cursor to the previous stop immediately with the latest pending reveal preserved, `w` changes wrap mode immediately and issues (or replaces) the keyed layout request for the new mode without releasing the existing gate, and a second resize is accepted, all without releasing the gate. Then release a layout matching the model's current parameters and assert the stop selected while the gate was held is revealed per the Issue 14 rules (the pending intent was preserved, not lost).
  ...
  - **Cached-file stale-layout navigation**: view file A at width 80, resize to 100 (layout requested for A only), navigate to cached file B (last prepared at width 80) with the layout worker gated → the model requests a layout for B at width 100, and the navigation intent (entry reveal or saved-viewport) is preserved; release the gate and assert the intent commits against B's width-100 layout per Issue 28's two-stage contract. Also test the fast path: navigate to a cached file that already has a matching installed layout → no layout request, intent commits immediately.
...
- [ ] Given navigation to a cached file whose installed layout does not match the current (path, revision, width, mode), then the model requests a prepared layout and the navigation intent commits when it installs; given a matching layout is already installed, then the intent commits without a request.
