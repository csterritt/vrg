...
- **Cause precedence**: each physical record contributes at most one integrity cause, chosen by the most-specific applicable row. A second `summary` contributes only `extra summary`, not `record after summary`. After the first valid `summary`, every other record contributes only `record after summary` and is not dispatched to a lifecycle parser, so it cannot mutate open-file state or create a later lifecycle cause. A trailing unterminated fragment after `summary` likewise contributes only the after-`summary` integrity cause, while retaining its malformed count; outside the post-summary state it contributes the unterminated-final-record cause and malformed count.
- **Issue 36 owns the post-summary transition**: its GREEN removes the current `context` exemption in `Builder.Add` and corrects the contradictory `"context after summary has no lifecycle effect"` row in `internal/searchindex/lifecycle_test.go`. Issue 44 begins from that green behavior and owns dedicated `context` regression/composed-outcome coverage and documentation, not a second parser change.
...
- **Dual representation is intentional**: when one physical record is both an integrity violation and independently counted record loss/warning, retain both representations in their respective components. In particular, a trailing unterminated record produces an integrity-cause line and increments the malformed aggregate; a malformed record after `summary` likewise produces the after-`summary` integrity cause and contributes to malformed record loss; a post-`summary` oversized record produces the after-`summary` cause and retains its oversized count (plus a recoverable-path detail when available); and a post-`summary` unknown-type record produces the after-`summary` cause and retains its unknown-type count/warning. Do not deduplicate any representation away.
...
- [ ] Given repeated identical integrity violations, then every occurrence produces its own line without aggregation, deduplication, or a cap; given a post-`summary` record that is also malformed, oversized, or unknown-type, then the after-`summary` cause and its independent malformed/oversized count or unknown-type count/warning both remain visible.
