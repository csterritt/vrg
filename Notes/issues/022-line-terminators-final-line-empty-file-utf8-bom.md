...
- **Manual**: search in a CRLF file → no `^M` shown and highlights land correctly; a file with a standalone CR mid-line shows `^M`; for the empty-file case use the Issue 4 fake-rg harness to emit a valid `begin`/`match` (line 1, `foo`)/`end`/`summary` stream for `a.txt` while `a.txt` is a zero-byte file on disk → the panel is empty, the gutter is three blank cells, no source rows (the stale note for this situation is Issue 29; before that lands, just an empty panel); a file with a UTF-8 BOM shows no `\uFEFF`/garbage and the first-line match is highlighted in the right place.
- **Automated**: FileBuffer tests: LF, CRLF, mixed; no final newline → last line counted; trailing newline → no phantom line; empty file → zero lines, one digit slot, gutter three cells wide, no source rows; mapping of terminator bytes to EOL column; span crossing text+terminator; UTF-8 BOM invisible with a first-line match at rg offset 0 mapping to raw byte 3 and highlighting the right cells; non-leading U+FEFF displayed/counted normally.
...
- [ ] Given an empty file, then the panel is empty with zero source lines and a three-cell gutter (one digit slot plus two spaces).
