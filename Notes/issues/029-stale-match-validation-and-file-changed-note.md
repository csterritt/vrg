...
- The note occupies the Issue 24 filename-row status slot: at constrained widths the path truncates to make room for it, nothing overflows, and layout dimensions stay nonnegative (composed-view verification below).
...
- The stale fallback target feeds the Issue 28 load-completion reveal: when the latest selected stop's first recorded submatch is dropped during a reload, the commit — once the reload's matching prepared layout installs, per Issue 28's two-stage contract — reveals the first **surviving** submatch (or the clamped fallback).
...
- **Automated**: FileBuffer tests: out-of-bounds range; same-length text replacement; one of two submatches surviving; all dropped but line exists → clamped start; line gone → last line; empty file; reload clearing/preserving the note; validation uses original/search bytes not stripped display text; CRLF terminator match still validates. App tests: the latest selected stop changes during a gated reload to a stop whose first recorded submatch is dropped by the new content → when the reload's matching prepared layout installs (two-stage path per Issue 28), the first survivor's row/cell is revealed; all-submatches-dropped stop with line present → clamped start revealed with no highlight; all-stale index → fixed exit status unchanged (outcome matrix row); composed filename-row test: the stale note with a long escaped path at ordinary and constrained widths (path truncated to make room for the note, nothing overflowing, nonnegative dimensions).
