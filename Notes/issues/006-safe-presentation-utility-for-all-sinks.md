...
- **Automated**:
  - Unit tests for path escaping (each rule), content escaping with byte→cell maps (a match covering an ESC byte highlights both `^` and `[`), standalone CR → `^M`, diagnostic escaping preserving line breaks and single-lining embedded filenames.
  - **Sink-safety table test.** A shared fixture set of hostile inputs: OSC (`\x1b]0;x\x07`), CSI (`\x1b[2J`), C0 (`\x07`, `\x08`, `\x1b`), C1 (`\xc2\x85`), DEL, standalone CR, invalid UTF-8 path bytes, embedded filename newline. For each existing sink (file list entry, filename rule, panel content, usage-error stderr), drive the fixture through the real composition path and assert on the **raw output** — *before* any ANSI stripping — that no fixture control byte survives verbatim. Do not verify by stripping ANSI and comparing; stripping would erase the very evidence being sought.
