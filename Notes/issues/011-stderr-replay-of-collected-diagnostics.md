...
- The controlled application-failure diagnostic from Issue 4 enters the session collection before shutdown and is replayed by the common post-restoration writer; from this issue on, the failure path performs no separate direct stderr write. Exactly-once holds across both mechanisms: the application-failure message appears once, after restoration, alongside any earlier diagnostics in collection order.
...
- [ ] Given a controlled application failure, then its diagnostic is replayed exactly once via the common post-restoration writer — never both written directly by the failure path and replayed from the collection.
...
- [ ] Given any controlled exit with collected diagnostics, then the replay bytes appear on stderr after the display-restoration sequence, and the terminal input modes are already restored when they appear.
