...
- Highlight and truncation boundaries must fall only on grapheme-cluster boundaries measured in cells. Mechanically enforce this boundary: no `utf8.DecodeRuneInString`-based display-width or truncation loop may remain outside the single shared grapheme/cell helper; byte decoding for purposes unrelated to display geometry is unaffected.
