...
- **Automated**: model tests with worker gates: A→B→C navigation with A's completion arriving while C is current → C's panel unchanged, A cached; re-entering A while its load is gated → no second load request; navigating back to A after completion shows cached content without a new load; completion after cancellation ignored.
