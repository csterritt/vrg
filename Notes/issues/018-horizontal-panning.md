...
The paintable-boundary maximum delivers the guarantee the simpler `E − 1` form intended: at the maximum offset, at least one **whole** cluster or marker cell of the widest visible line is painted whenever any cluster of that line fits the text width, so the text area is never entirely blank *because of clamping* and never shows half a glyph. The documented exception is preserved: when no cluster of the line fits the text width at all, the maximum is 0 and grapheme-safe clipping deliberately renders the in-window portion as blanks (Issue 19's geometric-reveal fallback accepts the same clipping).
...
- The painted-cluster guarantee constrains clamping, not the `_` gutter indicator (Issue 20): `_` means text is hidden to the **left**, which is independent of whether the line still paints clusters. Every visible line — including the widest — can simultaneously have hidden-left text (for example, uniform lines wider than the text area at a nonzero offset), so `_` may legitimately appear on every visible line at once. No rule in this issue constrains how many lines show `_`.
...
- [ ] Given the currently visible source lines, then at least one fully painted cluster or marker cell of the widest one is on screen at the maximum offset, except in the documented unpaintable-cluster case where no cluster of that line fits the text width (maximum 0, clipping blanks by design); an empty buffer, placeholder, or all-empty visible set clamps to 0.
