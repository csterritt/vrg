...
- **Re-entry sequence** (deterministic order, applying when a previously failed file is entered from a different file):
  ...
  4. A second failure appends exactly one new diagnostic occurrence to the open overlay per Issue 32's append rule (the reader's scroll position is preserved) and collects exactly one new occurrence for stderr replay (Issue 11). A success collects nothing new.
