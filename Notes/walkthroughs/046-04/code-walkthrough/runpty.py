...
Environment variables:
  ...
  VRG_TRIGGER_FILE  - if set, create this file after the initial output
                      settles and wait for a second settle before keys;
                      drives the tagged binary's VRG_TEST_DIAGNOSTIC_TRIGGER
                      watcher so the injected DiagnosticMsg is collected
                      after the SearchComplete diagnostics
"""
...
trigger_file = os.environ.get("VRG_TRIGGER_FILE", "")


def read_until_quiet(fd, output, quiet=1.0):
    """Append PTY output until no bytes arrive for `quiet` seconds."""
    while True:
        try:
...
if pid == 0:
    ...
    output = read_until_quiet(fd, b"")

    # Create the diagnostic-trigger file if requested, then wait for
    # the resulting work to settle so the DiagnosticMsg is collected
    # (after the SearchComplete diagnostics) before the quit keys.
    if trigger_file:
        with open(trigger_file, "w") as f:
            f.write("trigger")
        output = read_until_quiet(fd, output, quiet=0.5)
    ...
    for key in keys:
        ...
        try:
            ...
            break
        output = read_until_quiet(fd, output, quiet=0.3)

    # Read remaining output.
    output = read_until_quiet(fd, output)
