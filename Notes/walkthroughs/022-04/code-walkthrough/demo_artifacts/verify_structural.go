...
func main() {
	...
	// Strip a leading UTF-8 BOM to match ripgrep's behavior: rg
	// removes the BOM from searched line data, so submatch offsets
	// are relative to the line without the BOM.
	...
	fmt.Printf("LineCount=%d GutterWidth=%d BOMOffset=%d\n", buf.LineCount, buf.GutterWidth, buf.BOMOffset)
	for _, line := range buf.Lines {
		fmt.Printf("line %d: display=%q\n", line.Number, line.Display)
		fmt.Printf("  highlights=%v\n", line.Highlights)
		if len(line.Highlights) > 0 {
			fmt.Printf("  byteCells=%v\n", line.ByteCells)
		}
	}
}
