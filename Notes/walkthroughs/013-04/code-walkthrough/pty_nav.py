#!/usr/bin/env python3
"""Issue #13 walkthrough: n/p circular matched-line navigation.

Builds a fixture inside this walkthrough directory — a.txt (stops on
lines 1 and 15 of 60), b.txt (stops on lines 2 and 4 of 30), c.txt (a
stop on line 1) — then drives the built binary on a 100x24 pty. The
renderer diffs at cell level, so the raw byte stream is replayed through
a small screen emulator that also tracks the SGR underline attribute:
the current matched line's match is inverse+underline and the current
file-list entry is base+underline, so screen state proves where the
cursor sits:

  n — moves the underline through a.txt's stops, then across to b.txt
      and c.txt with the list underline following; after the last stop
      it wraps to the first.
  p — reverses, wrapping from the first stop to the last.
  scroll then n — manual scrolling leaves the cursor alone; n continues
      from the last selected stop without moving the viewport (reveal is
      Issue #14's).
"""
import fcntl
import os
...
def make_fixture():
    ...
    with open(os.path.join(FIXTURE, "a.txt"), "w") as f:
        f.write(numbered("a", 60, {1: "match a-one", 15: "match a-two"}))
    with open(os.path.join(FIXTURE, "b.txt"), "w") as f:
        f.write(numbered("b", 30, {2: "match b-one", 4: "match b-two"}))
    with open(os.path.join(FIXTURE, "c.txt"), "w") as f:
        f.write(numbered("c", 10, {1: "match c-one"}))
...
def describe(screen, ul):
    ...
    file, the underlined list entry, the underlined panel row's line
    number and text, and the panel's first content row."""
    rule = screen[0]
    cur_file = ""
    ...
    return cur_file, list_ul, match_desc, screen[1][PANEL:].strip()


def expect(s, key, label, want_file, want_list, want_match, want_top=None):
    """Send key, settle, and assert the cursor's visible position."""
    screen, ul = s.send_settle(key)
    cur_file, list_ul, match_desc, top = describe(screen, ul)
...
def main():
    ...
    # Startup selects the first stop in path-then-line order: a.txt's
    # line 1 is the current matched line and a.txt's entry is underlined.
    # (The match text is split by styling escapes, so the wait needle is
    # the last file-list entry instead.)
    s.wait_for(b"./c.txt", "initial frame")
    screen, ul = s.settle()
    expect0 = describe(screen, ul)
    cur_file, list_ul, match_desc, top = expect0
    if cur_file != "a.txt" or list_ul != "a.txt" or "match a-one" not in match_desc:
        s.kill()
        fail("init: %r" % (expect0,))
    print("%-10s: file=%s list=%s underline=%s top=%r"
          % ("init", cur_file, list_ul, match_desc, top))

    # n walks a.txt's stops, then crosses into b.txt and c.txt — the
    # panel switches, the uncached file loads, and the list underline
    # follows the cursor's file.
    expect(s, b"n", "n", "a.txt", "a.txt", "match a-two")
    expect(s, b"n", "n→b", "b.txt", "b.txt", "match b-one")
    expect(s, b"n", "n", "b.txt", "b.txt", "match b-two")
    expect(s, b"n", "n→c", "c.txt", "c.txt", "match c-one")

    # The last stop's n wraps circularly to the first; p on the first
    # wraps back to the last; p again retreats into b.txt's last stop.
    expect(s, b"n", "n(wrap)", "a.txt", "a.txt", "match a-one")
    expect(s, b"p", "p(wrap)", "c.txt", "c.txt", "match c-one")
    expect(s, b"p", "p", "b.txt", "b.txt", "match b-two")

    # Back to a.txt's first stop for the scroll-independence check.
    expect(s, b"p", "p", "b.txt", "b.txt", "match b-one")
    expect(s, b"p", "p", "a.txt", "a.txt", "match a-two")
    expect(s, b"p", "p", "a.txt", "a.txt", "match a-one")

    # Manual scrolling moves only the viewport — top becomes line 11 —
    # never the cursor. The following n continues from the last
    # selected stop (a.txt line 1 → line 15) and leaves the scrolled
    # viewport in place: no reveal is Issue #14's.
    for _ in range(10):
        s.send(DOWN)
    screen, ul = s.settle()
    cur_file, list_ul, match_desc, top = describe(screen, ul)
    if "a row 11" not in top:
        s.kill()
        fail("scroll: first content row %r, want 'a row 11' at top 10" % top)
    print("%-10s: file=%s list=%s underline=%s top=%r"
          % ("scroll↓10", cur_file, list_ul, match_desc, top))
    expect(s, b"n", "n", "a.txt", "a.txt", "match a-two", want_top="a row 11")
