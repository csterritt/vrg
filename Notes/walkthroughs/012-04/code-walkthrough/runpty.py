#!/usr/bin/env python3
"""Run vrg with a PTY, wait for output to settle, send keys, capture
stdout (from the PTY) and stderr (from a separate file) separately.
...
Environment variables:
  ...
  VRG_STDERR   - path to file for capturing stderr (default: stderr.txt)
"""
...
# Open a file for stderr capture.
stderr_fd = os.open(stderr_file, os.O_WRONLY | os.O_CREAT | os.O_TRUNC, 0o644)

pid, fd = pty.fork()
if pid == 0:
    # Child: redirect stderr to the file, stdout/stdin to the PTY.
    os.dup2(stderr_fd, 2)
    os.close(stderr_fd)
