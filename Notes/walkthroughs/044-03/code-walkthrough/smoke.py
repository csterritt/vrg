#!/usr/bin/env python3
"""Issue #44 smoke harness: post-summary context under a PTY.

Drives the built vrg binary through the issue's manual scenario with
the Issue #4 fake-rg fixture family (handshake side file) and a
separated stderr pipe, plus the neighbouring contract:

  1. the issue's manual check: valid begin/match/end records, then
     summary, then a context record, child exit 0 -> the outcome is a
     stream-integrity failure (the fatal path per the outcome
     matrix): the overlay names "record after summary", dismissal
     reveals browse, the session exits 2, and the same diagnostic is
     replayed to stderr - the context is never silently accepted.
  2. the surviving exemption: context records before begin, inside
     the file block, and between end and summary -> a complete
     stream, clean browse, exit 0, no diagnostics.
...
def context(p):
    return ('{"type":"context","data":{"path":{"text":"%s"},'
            '"lines":{"text":"nearby\\n"},"line_number":2,'
            '"submatches":[]}}' % p)
...
def scenario_context_after_summary():
    """The issue's manual check: valid records, summary, then context.

    begin/match/end for test.txt, the summary, then a context record,
    child exit 0. Under the summary-is-final contract the context is a
    stream-integrity failure like any other post-summary record:
    usable results exist, so the fatal outcome is the error overlay
    over browse - the first q dismisses it, the second quits at the
    fixed status 2. The overlay names 'record after summary' - never a
    manufactured 'ripgrep exited with code 0' line - and the same
    diagnostic is replayed to stderr after terminal restoration."""
    run, hs = launch("s1",
                     [begin("test.txt"), match("test.txt"),
                      end("test.txt"), summary(), context("test.txt")],
                     [], 0)
    try:
        wait_file(hs)
        run.wait_output("record after summary")
    ...
    check("post-summary-context exit 2", run.exit_code == 2,
          "exit=%s" % run.exit_code)
    check("post-summary-context overlay names the cause",
          "record after summary" in visible,
          "visible=%r" % visible)
    check("post-summary-context emits no code-0 process line",
          "exited with code 0" not in visible,
          "visible=%r" % visible)
    check("post-summary-context dismissal reveals browse",
          "test.txt" in visible)
    check("post-summary-context stderr replays the cause",
          "record after summary" in run.stderr(),
          "stderr=%r" % run.stderr())
    check_terminal_restored("post-summary-context", run)


def scenario_context_before_summary():
    """The surviving exemption: context in every pre-summary position.

    A context ahead of the first begin, one inside the file block, and
    one between end and summary, child exit 0: the stream stays
    complete - no overlay, no integrity cause - and the run browses
    test.txt and exits 0 on q."""
    run, hs = launch("s2",
                     [context("test.txt"), begin("test.txt"),
                      match("test.txt"), context("test.txt"),
                      end("test.txt"), context("test.txt"), summary()],
                     [], 0)
    try:
        wait_file(hs)
        run.wait_output("test.txt")
        run.send("q")  # clean browse: quits at the ordinary status 0
        run.wait_exit()
        run.drain()
    except Exception:
        run.kill()
    ...
    visible = strip_ansi(run.text())
    check("pre-summary-context exit 0", run.exit_code == 0,
          "exit=%s" % run.exit_code)
    check("pre-summary-context opens no overlay",
          "record after summary" not in visible
          and "missing summary" not in visible,
          "visible=%r" % visible)
    check("pre-summary-context browses the file",
          "test.txt" in visible)
    check("pre-summary-context stderr stays silent",
          run.stderr().strip() == "",
          "stderr=%r" % run.stderr())
    check_terminal_restored("pre-summary-context", run)
...
def main():
    ...
    run_scenario("scenario 1: valid records, summary, then context, exit 0",
                 scenario_context_after_summary)
    run_scenario("scenario 2: context records before summary, exit 0",
                 scenario_context_before_summary)
