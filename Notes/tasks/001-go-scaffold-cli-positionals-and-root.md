...
Add a non-help row for the help assignment spellings `--help=false` and `-h=false` before `--` with a valid pattern: assert they do **not** produce the help-only result — no help on stdout, result kind ≠ help-only, and the help-only sentinels (no root validation, child startup, TUI initialization, or stub-via-help dispatch) are not triggered — without pinning the eventual exit code, which Issue #2 later changes to 2. This pins the Issue #1 contract that the preflight classifies help on exact token spelling, not a prefix or substring of `--help`/`-h`, so a permissive matcher cannot silently emit help where the issue forbids it.

Test the approved Task 1 output strategy across every possible `mow.cli` emission point: first-token help, parser-failure `Error:` plus help, and module-requested help. Require no native library bytes or duplicate diagnostics on process stderr, exactly one sanitized help copy on stdout for help-only paths, and on each usage error the module's specific sanitized single-line diagnostic followed by one generated usage block — module-rendered help text on stderr is required, library-emitted usage is forbidden. Cover hostile operand and executable-name substitutions, invalid bytes, and terminal controls while preserving legitimate help line breaks. For containment, add deterministic tests for serialization, capture drainage beyond pipe capacity, and restoration after success and failure; for prevention, prove preflight classifies every help and invalid-input path before any emitting library call and that help renders from the parser's shared metadata.
...
### 6. Review implementation against the recorded decision

**Type**: REVIEW
**Output**: Human approval confirms the implementation and tests satisfy Issue #1 and the architecture recorded in Task 1 before Issue #2 starts.
**Depends on**: 5
