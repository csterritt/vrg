...
Revision 3. Supersedes revision 2 and incorporates the decisions made during the interview addressing critique 2. This document specifies version 1; acknowledged limitations are not promises of future features.
...
17. As a user, I want malformed and oversized records skipped and counted, so that a damaged record does not necessarily discard the rest of the search.
...
32. As a user, I want terminals narrower than 20 columns or shorter than 3 rows to show “Terminal too small” and automatically recover on resize, so that small sizes degrade predictably.
...
37. As a user, I want a late failure for a non-current file recorded without interrupting me with an overlay, so that moving on is respected.
...
41. As a user, I want reload to preserve reading position unless I navigate during the load, in which case the latest selected match is revealed, so that asynchronous completion follows my latest intent.
42. As a user, I want repeated load requests for a file already loading to be ignored, so that reload and rapid navigation do not create overlapping reads for that file.
...
50. As a user, I want startup to select and reveal the first match on the first matched line, with the first `n` advancing to the second stop, so that browsing starts at the beginning.
...
57. As a user, I want a revisited file's saved vertical viewport used as the starting point, with destination-match revelation taking precedence, so that revisits preserve context when possible without hiding the destination.
...
59. As a user, I want each pop-up to last one second or until a key press, with that key also performing its normal action, so that pop-ups never delay navigation or quitting.
...
73. As a user, I want LF and CRLF removed from display but retained in byte-coordinate mapping, with terminator-only matches displayed at end of line, so that newline handling does not invalidate valid matches.
...
79. As a user, I want help to accept only scrolling, its documented dismissal keys, and `ctrl+c`, so that keys do not act on content behind it.
...
84. As a user, I want `q` to dismiss an overlay before quitting ordinary browsing, and `ctrl+c` always to exit 130, so that modal and emergency exit behavior are predictable.
85. As a user, I want documented independent scale examples and memory limitations rather than an aggregate-capacity guarantee, so that I can judge whether a search is suitable for my machine.
...
- Read full files asynchronously on first view. Retain successful buffers for the session; no eviction. At most one load is in flight per raw path. Requesting `r` or re-entering a path already loading does not start another load.
- Load completion is associated with the requested raw path and request identity. It updates only that path's cache/status. It may affect the visible panel only when that path is current; reveal the **latest** selected cursor target, never a target captured from an earlier selection. Late messages after cancellation cannot revive the UI.
...
- Map removed terminator bytes and zero-width positions through the end of the original line to the display end-of-line position. For example, `$` at byte 4 of `hit\r\n` maps to display column 3. A match solely on removed terminator bytes becomes an end-of-line marker; a span also covering visible text highlights that visible text.
...
- In run-off-edge mode, if the target cell is hidden horizontally, move by the minimum number of columns needed to make it visible. If already visible, retain the offset. Oversized matches are revealed by showing their start cell; full-span visibility is not required. Indicators still treat any genuinely visible portion of a span as visible.
- On file change, use saved per-file vertical state as the starting viewport and then apply destination reveal. Reset horizontal offset to zero and then apply horizontal reveal. There are no stories or tests for hypothetical future selection routes.
...
- The fixed terminal minimum is 20 columns and 3 rows. Below either, show centered “Terminal too small” as space permits. Only `q` and `ctrl+c` are active: `q` exits with the applicable outcome (130 if searching), and `ctrl+c` exits 130. Preserve selection, modal state, and visibility preference for recovery on resize. Do not let this screen trap the user behind an invisible modal overlay.
...
- File-change pop-ups start at selection, are centered, and show a single-line safe path left-truncated to fit. Each has its own fresh one-second lifetime; a stale timer cannot dismiss a newer instance. Any key dismisses a pop-up and is routed normally in the same action.
...
- **Asynchronous App behavior**: controlled A→B→C completions; late success never replacing C; late failure diagnostic-only; entry into an in-flight path; latest same-file target applied at completion; explicit reload preserving anchor; navigation during reload overriding it; ignored duplicate `r`; failed reload hiding old data; retry on entry/`r` but not same-file steps; unsupported-file reload; one-stop retry via `r`; pop-up timing at selection independent of load timing.
- **Overlays/output**: modal key routing and global interrupt; error suspending/restoring scrolled help; append preserving scroll; long unbroken diagnostic wrapping; pop-up truncation, fresh timers, dismissal-plus-action; too-small exit behavior; sanitized filenames/control sequences in all sinks; stderr replay including unseen diagnostics exactly once per occurrence, with readable diagnostic line breaks and escaped filename newlines.
