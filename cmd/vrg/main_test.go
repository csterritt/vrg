...
// boundary. It is built with the vrg_testhooks tag (Issue #45) so the
// test-only VRG_TEST_* seams are available to the subprocess tests
// while the untagged production binary stays free of them.
var binPath string
...
func assertHelpRun(t *testing.T, res runResult, args []string) {
	...
	if strings.Contains(res.stdout, "Searching") {
		t.Fatalf("vrg %v: help path produced search output: %q", args, res.stdout)
	}
...
// Search cases are covered by subprocess tests in search_test.go; this
// test covers the usage-error boundary at the process level.
